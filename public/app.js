document.addEventListener('DOMContentLoaded', () => {
  ...
  function startMultiPlayer() {
    ...
    // On Timeout
    socket.on('timeout', () => {
      infoDisplay.innerHTML = 'La sesión expiró'
    })
  ...
  function rotate() {
    if (isHorizontal) {
      ...
      submarine.classList.remove('horizontal');
      ...
      battleship.classList.remove('horizontal');
  ...
  function checkForWins() {
    let enemy = 'computer'
    if(gameMode === 'multiPlayer') enemy = 'enemigo'
    if (destroyerCount === 2) {
      infoDisplay.innerHTML = `Hundiste el destructor ${enemy} `
      destroyerCount = 10
    }
    if (submarineCount === 3) {
      infoDisplay.innerHTML = `Hundiste el submarino ${enemy}`
      submarineCount = 10
    }
    if (cruiserCount === 3) {
      infoDisplay.innerHTML = `Hundiste el crucero ${enemy}`
      cruiserCount = 10
    }
    if (battleshipCount === 4) {
      infoDisplay.innerHTML = `Hundiste el barco de guerra ${enemy}`
      battleshipCount = 10
    }
    if (carrierCount === 5) {
      infoDisplay.innerHTML = `Hundiste el transportador ${enemy}`
      carrierCount = 10
    }
    if (cpuDestroyerCount === 2) {
      infoDisplay.innerHTML = `El ${enemy} hundió tu destructor`
      cpuDestroyerCount = 10
    }
    if (cpuSubmarineCount === 3) {
      infoDisplay.innerHTML = `El ${enemy} hundió tu submarino`
      cpuSubmarineCount = 10
    }
    if (cpuCruiserCount === 3) {
      infoDisplay.innerHTML = `El ${enemy} hundió tu crucero`
      cpuCruiserCount = 10
    }
    if (cpuBattleshipCount === 4) {
      infoDisplay.innerHTML = `El ${enemy} hundió tu barco de guerra`
      cpuBattleshipCount = 10
    }
    if (cpuCarrierCount === 5) {
      infoDisplay.innerHTML = `El ${enemy} hundió tu transportador`
      cpuCarrierCount = 10
    }
